...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (error instanceof RiotApiError) {
      // When the API key is missing, expired, or invalid, return a user-friendly
      // message instead of exposing the internal error details.
      // - 401/403: Riot API rejects an expired or invalid key
      // - 500 with "RIOT_API_KEY": riotFetch throws because the env var is not configured
      if (
        error.status === 401 ||
        error.status === 403 ||
        (error.status === 500 && error.message.includes("RIOT_API_KEY"))
      ) {
        return Response.json(
